...
const CircleButton = ({ classname, children, OnClick, onText }: { classname?: string, children: React.ReactNode, OnClick?: () => void, onText?: string }) => {
    return (
        ...
        <div onClick={OnClick} className={cn("h-6 w-6 sm:h-9 sm:w-9 rounded-full md:w-9 md:h-9 lg:h-10 lg:w-10 flex items-center justify-center hover:border hover:border-sky-300 bg-4 transform transition duration-200 hover:scale-150 hover:shadow-lg hover:bg-1 hover:text-white text-2 relative group", classname)}>
